...
export default function TelegramPan() {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const response = await axios.post('/utilities/telegram-search', { type: 'pan', input: inputValue });
            if (response.data.success) {
                setResult(response.data.details);
            } else {
                setError(response.data.message || 'Details not found for this PAN.');
            }
        } catch (err) {
            ...
            header={
                <div className="flex flex-col">
                    <h1 className="text-xl font-bold text-gray-800 dark:text-white leading-tight">
                        PAN Card Details Lookup
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                        Instantly retrieve details associated with a PAN Card
                    </p>
                </div>
            }
        >
            <Head title="PAN Card Lookup" />
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="p-8">
                        ...
                        <p className="text-center text-slate-500 mb-8 font-medium">
                            Enter a 10-character PAN number to fetch the associated details instantly.
                        </p>
