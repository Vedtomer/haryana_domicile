...
export default function RcPdf() {
    ...
    const [vechilNo, setVechilNo] = useState('');
    ...
    const handleSearch = async (e) => {
        e.preventDefault();
        const clean = vechilNo.trim().toUpperCase().replace(/[\s-]/g, '');
        if (!clean) {
        ...
        setResult(null);
        try {
            const response = await axios.post('/utilities/rc-pdf-instant/search', { vechil_no: clean });
    ...
    const downloadPdf = () => {
        ...
        if (result.file_url || result.pdf_url) {
            window.open(result.file_url || result.pdf_url, '_blank');
            return;
        }
        ...
        if (pdfData) {
            ...
            link.href = pdfData;
            link.download = `RC_${vechilNo.replace(/[\s-]/g, '')}.pdf`;
            ...
            header={
                <div className="flex flex-col">
                    ...
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                        Download Vehicle RC PDF instantly
                    </p>
                </div>
            }
        >
            <Head title="Rc Pdf Instant" />
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="p-8">
                        ...
                        <p className="text-center text-slate-500 mb-8 font-medium">
                            Enter Vehicle Number to fetch RC PDF.
                        </p>
                        ...
                        <form onSubmit={handleSearch} className="space-y-5">
                            <div>
                                ...
                                <input
                                    ...
                                    value={vechilNo}
                                    onChange={(e) => {
                                        setVechilNo(e.target.value.toUpperCase());
                                    }}
                                    placeholder="e.g. HR26DD1234"
                            ...
                            <button
                                type="submit"
                                disabled={loading || !vechilNo.trim()}
                            ...
                            >
                                {loading ? (
                                    ...
                                    <>
                                        ...
                                        Get RC PDF ({currentService?.coin_cost ?? 99} Coins)
                                    </>
                                )}
                            </button>
                        </form>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 overflow-hidden animate-in fade-in zoom-in duration-300">
                        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 p-5 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-white font-black text-xl tracking-widest">{vechilNo.toUpperCase()}</p>
                            </div>
                        </div>
                        
                        {(result.file_url || result.pdf_url || result.pdf || result.data?.pdf || result.base64) && (
                        ...
                        <div className="p-6 space-y-1">
                            <InfoRow label="Vehicle Number"  value={vechilNo.toUpperCase()} icon="directions_car" />
                            {result.data?.owner_name && <InfoRow label="Owner Name" value={result.data.owner_name} icon="person" />}
                            {result.data?.maker_model && <InfoRow label="Model" value={result.data.maker_model} icon="build" />}
                        </div>
                    </div>
                )}
            </div>
