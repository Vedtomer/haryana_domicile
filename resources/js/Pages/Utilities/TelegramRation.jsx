...
export default function TelegramRation() {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const response = await axios.post('/utilities/telegram-search', { type: 'ration', input: inputValue });
            if (response.data.success) {
                setResult(response.data.details);
            } else {
                setError(response.data.message || 'Details not found for this card.');
            }
        } catch (err) {
            ...
            header={
                <div className="flex flex-col">
                    <h1 className="text-xl font-bold text-gray-800 dark:text-white leading-tight">
                        Ration Card Details Lookup
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                        Instantly retrieve details associated with a Ration Card
                    </p>
                </div>
            }
        >
            <Head title="Ration Card Lookup" />
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="p-8">
                        ...
                        <p className="text-center text-slate-500 mb-8 font-medium">
                            Enter a Ration Card number to fetch the associated details instantly.
                        </p>
                        ...
                        <form onSubmit={handleSearch} className="space-y-5">
                            ...
                            <button
                                type="submit"
                                disabled={loading || inputValue.length < 8}
