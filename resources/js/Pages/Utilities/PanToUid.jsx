            ...
            header={
                <div className="flex flex-col">
                    <h1 className="text-xl font-bold text-gray-800 dark:text-white leading-tight">
                        Pan To Uid Advance Instant
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                        Get advanced UID details instantly using PAN
                    </p>
                </div>
            }
        >
                            ...
                            >
                                {loading ? (
                                    ...
                                    <>
                                        ...
                                        Get UID Details ({currentService?.coin_cost ?? 149} Coins)
                                    </>
                                )}
                            </button>
                        </form>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 overflow-hidden animate-in fade-in zoom-in duration-300">
                        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 p-5 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-white font-black text-xl tracking-widest">{result.uid || result.aadhar_number || result.aadhaar_number || result.aadhar || 'Data Found'}</p>
                            </div>
                            <span className="ml-auto px-3 py-1 bg-green-400 text-green-900 text-xs font-black rounded-full uppercase">Active</span>
                        </div>
                        <div className="p-6 space-y-1">
                            <InfoRow label="UID / Aadhaar"       value={result.uid || result.aadhar_number || result.aadhaar_number || result.aadhar} icon="badge" />
                            <InfoRow label="PAN Number"          value={result.pan || result.pan_number || pan}      icon="credit_card" />
