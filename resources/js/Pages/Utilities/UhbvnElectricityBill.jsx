...
export default function UhbvnElectricityBill() {
    ...
    const handleDownload = async (e) => {
        ...
        const targetUrl = `/utilities/uhbvn-electricity-bill/download?uid=${digitsOnly}`;
        ...
        try {
            ...
            if (response.ok && contentType.toLowerCase().includes('application/pdf')) {
                ...
                a.href = blobUrl;
                a.download = `UHBVN_Electricity_Bill_${digitsOnly}.pdf`;
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                window.URL.revokeObjectURL(blobUrl);

                setSuccessMessage(`UHBVN Electricity Bill downloaded successfully for Account ${digitsOnly}!`);
            } else {
                setErrorMessage(`Bill not found on UHBVN for Account Number "${digitsOnly}". Please check your Account Number.`);
            }
        } catch (err) {
        ...
        <AdminLayout header={
            <div className="flex items-center gap-3">
                ...
                <h2 className="font-semibold text-xl text-gray-800 dark:text-slate-200 leading-tight">
                    UHBVN Electricity Bill Download
                </h2>
            </div>
        }>
            <Head title="UHBVN Electricity Bill" />
                ...
                <Paper elevation={0} className="border border-slate-200/60 dark:border-slate-800 shadow-md p-8 rounded-2xl overflow-hidden relative bg-white dark:bg-slate-900">
                    ...
                    <div className="relative z-10 flex flex-col items-center max-w-md mx-auto text-center">
                        ...
                        <form onSubmit={handleDownload} className="w-full flex flex-col gap-4">
                            <TextField
                                fullWidth
                                label="UHBVN Account Number (UID)"
                                variant="outlined"
                                value={accountNumber}
                                onChange={(e) => {
                            ...
                            >
                                {isDownloading ? 'Downloading UHBVN Bill...' : 'Download UHBVN Bill'}
                            </Button>
                        </form>
                        ...
                        <Alert severity="info" className="mt-8 text-left rounded-xl bg-blue-50/50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900">
                            <strong>Note:</strong> This service is provided directly via UHBVN portal (Uttar Haryana) and is completely free of charge. No coins will be deducted.
                        </Alert>
                    </div>
                </Paper>
            </div>
