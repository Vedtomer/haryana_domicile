        ...
        <AdminLayout>
            ...
            {addingCoinsTo && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <form onSubmit={handleAddCoins} className="bg-white rounded-2xl p-8 shadow-2xl w-full max-w-md">
                        ...
                        <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100">
                            ...
                            <div>
                                <p className="text-base font-bold text-slate-900">{addingCoinsTo.name || <span className="italic text-slate-400">No Name</span>}</p>
                                {addingCoinsTo.email && <p className="text-sm text-slate-500">{addingCoinsTo.email}</p>}
                                {addingCoinsTo.phone && <p className="text-sm text-slate-400">{addingCoinsTo.phone}</p>}
                            </div>
                        </div>

                        <h3 className="text-lg font-bold text-slate-900 mb-5">Add Coins</h3>

                        {/* Trial / Paid Toggle */}
                        <div className="mb-5">
                            <label className="block text-sm font-medium text-slate-700 mb-3">Coin Type</label>
                            ...
                            <p className="mt-2 text-xs text-slate-400">
                                {coinType === 'trial' ? 'These are complimentary trial coins for the user to explore.' : 'These are purchased/paid coins being credited manually.'}
                            </p>
                        </div>
                        ...
                        <div className="flex justify-end gap-3">
                            <button type="button" onClick={() => { setAddingCoinsTo(null); setAmount(''); setCoinType('trial'); }} className="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition-colors">Cancel</button>
                            <button type="submit" className={`px-5 py-2.5 text-sm font-bold text-white rounded-xl shadow hover:shadow-md transition-all ${
