                                ...
                                {records.data.map(record => (
                                    <tr key={record.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                        ...
                                        <td className="p-4 text-right space-x-3">
                                            ...
                                            <button 
                                                onClick={() => {
                                                    if (confirm('Are you sure you want to delete this PAN Card? This cannot be undone.')) {
                                                        import('@inertiajs/react').then(({ router }) => {
                                                            router.delete(`/admin/manual-pan-cards/${record.id}`);
                                                        });
                                                    }
                                                }}
                                                className="inline-flex items-center gap-1.5 text-sm font-medium text-red-600 hover:text-red-700 dark:text-red-500 dark:hover:text-red-400"
                                            >
