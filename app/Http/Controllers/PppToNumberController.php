...
class PppToNumberController extends Controller
{
    public function index()
    {
        $service = Service::where('slug', 'ppp-to-number')->first();
    ...
    public function search(Request $request)
    {
        ...
        if (!empty($apiUrl)) {
            try {
                ...
                if ($response->successful()) {
                    ...
                    if (!empty($members) && is_array($members)) {
                        $members = array_map(fn ($m) => [
                            ...
                            'status'   => $m['status'] ?? $m['verificationStatus'] ?? null,
                        ], $members);
                ...
                Log::error('PppToNumberController custom API error: ' . $e->getMessage());
            }
        }

        // 2. Fallback: try meraparivar.haryana.gov.in portal endpoint
        try {
    ...
    public function updateApi(Request $request)
    {
        ...
        Setting::set('ppp_to_number_url', trim($request->input('api_url')));
        Setting::set('ppp_to_number_key', trim((string) $request->input('api_key', '')));
    ...
    private function deductAndLog($user, $service, int $coinCost, bool $isStaff, string $familyId, int $memberCount): void
    {
        if (!$isStaff && $coinCost > 0) {
            $user->deductCoins($coinCost, CoinTransaction::TYPE_SERVICE_DEDUCTION, 'PPP to Number: ' . $familyId);
        }
        ...
        try {
            ServiceRequest::create([
                ...
                'service_name'  => $service ? $service->name : 'PPP to Number',
                'input_data'    => ['Family ID (PPP)' => $familyId, 'Members Found' => $memberCount],
                'coins_charged' => $isStaff ? 0 : $coinCost,
                'status'        => ServiceRequest::STATUS_COMPLETED,
                'completed_at'  => now(),
            ]);
        } catch (\Throwable $e) {
            Log::error('PppToNumber ServiceRequest log error: ' . $e->getMessage());
        }
    }
}
